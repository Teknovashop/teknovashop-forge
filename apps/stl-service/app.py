...
from models import REGISTRY  # <- usamos REGISTRY, NO build_model
...
def _apply_holes(mesh: trimesh.Trimesh, holes: List[Dict[str, float]], thickness: float) -> trimesh.Trimesh:
    ...
    'holes': [{x_mm, y_mm, d_mm}]  (diámetro en mm)
    """
    if not holes:
        return mesh
    ...
    for h in holes:
        ...
        d = max(0.1, float(h.get("d_mm", 1.0)))
        r = d * 0.5
        height = max(thickness * 2.5, 5.0)
        cyl = cylinder(radius=r, height=height, sections=64)

        # Centramos el cilindro para que atraviese la pieza en Z
...
class GenerateParams(BaseModel):
    ...
    holes: List[Dict[str, float]] = Field(default_factory=list)  # [{x_mm,y_mm,d_mm}]
...
async def generate(payload: GeneratePayload):
    ...
    # textOps: placeholder (se habilita cuando tengamos fuente y extrusión tipográfica robusta)
    ...
    # Homogeneizamos el nombre para el frontend (acepta stl_url | signed_url | url)
    stl_url = (up or {}).get("stl_url") or (up or {}).get("signed_url") or (up or {}).get("url")
    return {"ok": True, "slug": slug, "stl_url": stl_url, **(up or {})}
