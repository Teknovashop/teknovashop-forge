...
class GenerateReq(BaseModel):
    model: ModelKind
    holes: Optional[List[Any]] = None  # aceptamos dicts, tuplas o HoleSpec
...
def _hole_to_dict(h: Any) -> Optional[Dict[str, float]]:
    """
    Devuelve SIEMPRE dict {x_mm, z_mm, d_mm} para ser compatible
    con los modelos que usan h.get(...).
    Acepta:
    ...
    if isinstance(h, (list, tuple)) and len(h) >= 3:
        return {"x_mm": _as_float(h[0]), "z_mm": _as_float(h[1]), "d_mm": _as_float(h[2])}
    if isinstance(h, dict):
        ...
        if x is None or z is None or dval is None:
            return None
        return {"x_mm": _as_float(x), "z_mm": _as_float(z), "d_mm": _as_float(dval)}
    # HoleSpec?
    if hasattr(h, "x_mm") and hasattr(h, "z_mm") and hasattr(h, "d_mm"):
        return {"x_mm": _as_float(h.x_mm), "z_mm": _as_float(h.z_mm), "d_mm": _as_float(h.d_mm)}
    return None

def _normalize_holes_dicts(holes: Optional[List[Any]]) -> List[Dict[str, float]]:
...
def generate(req: GenerateReq):
    try:
        ...
        # Normaliza agujeros a LISTA DE DICTS (compatibles con tus modelos)
        holes_dicts = _normalize_holes_dicts(req.holes)
        ...
        if holes_dicts:
            print(f"[generate] holes[0] sample={repr(holes_dicts[0])} type={type(holes_dicts[0]).__name__}")
